...
@export_storage var playerInsectors: Array[BattleBoardUnitClientEntity]:
	get:
		playerInsectors.clear()
		for insector in turnBasedEntities:
			if insector is BattleBoardUnitClientEntity and insector.factionComponent.factions == FactionComponent.Factions.player1:
				playerInsectors.append(insector)
		return playerInsectors

@export_storage var enemyInsectors: Array[BattleBoardUnitClientEntity]:
	get:
		enemyInsectors.clear()
		for insector in turnBasedEntities:
			print("LOOKING FOR ENEMY UNIT...")
			print(playerInsectors)
			if insector is BattleBoardUnitClientEntity and insector.factionComponent.factions == FactionComponent.Factions.player2:
				print(insector.factionComponent.factions)
				enemyInsectors.append(insector)
		return enemyInsectors

@export_storage var currentTeamParty: Array[BattleBoardUnitClientEntity]:
	get:
...
func _ready() -> void:
	...
	NetworkBattleBoard.teamChanged.connect(_onTeamChanged)

func _onTeamChanged(newTeam: FactionComponent.Factions) -> void:
	print("TEAM CHANGE CLIENT STATE RESET")
	currentTeam = newTeam
	print("CURRENT PARTY")
...
func isTeamExhausted(team: int = currentTeam) -> bool:
	var party: Array[BattleBoardUnitClientEntity] = playerInsectors if team == FactionComponent.Factions.player1 else enemyInsectors
	for unit in party:
