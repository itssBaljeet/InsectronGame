...
func loadSceneAndAddInstance(path: String, parent: Node, position: Vector3 = Vector3.ZERO) -> Node:
...
## Returns: The new instance.
func addSceneInstance(scene: PackedScene, parent: Node, position: Vector3 = Vector3.ZERO) -> Node:
	if scene == null:
	...
	if newChild is Node3D or newChild is Control: newChild.position = position
