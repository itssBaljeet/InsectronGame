...
class_name Entity
extends Node3D # An "entity" would always have a visual presence, so it cannot be just a [Node].
...
func spawnPath(scenePath: String, positionOffset: Vector3 = Vector3.ZERO, copyZIndex: bool = false) -> Node:
...
func spawnNode(node: Node, positionOffset: Vector3 = Vector3.ZERO, copyZIndex: bool = false) -> Node:
	var myParent: Node = self.get_parent()
	if not myParent:
