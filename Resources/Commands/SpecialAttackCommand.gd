...
func _playVFX(context: BattleBoardContext, origin: Vector3i, target: Vector3i) -> void:
	...
	match attack.attackType:
		...
		AttackResource.AttackType.AREA:
			...
			vfx.scale = Vector3.ONE * attack.vfxScale * sqrt(areaSize / 9.0)
			match attack.attackName:
				"Shockwave":
					vfx = vfx as GPUParticles3D
					vfx.emitting = true
					vfx.restart()
					print("Emitting: ", vfx.emitting)
	...
	var duration := 1.5
	if attack.attackType == AttackResource.AttackType.PIERCING:
		duration = 2.0  # Longer for cone attacks
	elif attack.attackType == AttackResource.AttackType.AREA:
		duration = 1.0
	
	await context.board.get_tree().create_timer(duration).timeout
	vfx.queue_free()
